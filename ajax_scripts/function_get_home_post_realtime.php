...
if(isset($_GET['action']) && $_GET['action'] == "get_posts_realtime"){
...
$row_time = $sql_get_posts_time->fetch(PDO::FETCH_ASSOC);
...
$sql_get_post = $db->prepare("SELECT * FROM posts WHERE FIND_IN_SET(posted_by, :list) AND time=:time_last LIMIT 1");


$sql_get_post->execute(array("list" => $list, "time_last" => $time_last));

$numrow_post = $sql_get_post->rowCount();

if($numrow_post < 1){

}elseif($numrow_post >= 1){


$row_post = $sql_get_post->fetch(PDO::FETCH_ASSOC);
